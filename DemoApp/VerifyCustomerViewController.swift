...
class VerifyCustomerViewController: UIViewController, UITextFieldDelegate {
...
//        let colorBottom = UIColor(red: 80/255.0, green: 177/255.0, blue: 186/255.0, alpha: 1.0).cgColor
    ...
    func validateFields() -> Bool {
        if cardTextField.text == "", mobNumberTextField.text == "" {
            showAlert("Validation alert", message: "Please enter your 16-digit card number or your mobile number")
            return false
        }else {
            if cardTextField.text != "" {
                if (cardTextField.text?.count)! < 19 {
                    showAlert("Validation alert", message: "Please enter valid card number")
                    return false
                }else if mobNumberTextField.text != "" {
                    if (mobNumberTextField.text?.count)! < 10 {
